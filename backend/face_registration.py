...
class AttendanceSystem:
    def __init__(self, dataset_dir="registered_faces", attendance_file="attendance.csv",
                 tolerance=None, face_db_url=None):
        self.dataset_dir = dataset_dir
        self.attendance_file = attendance_file
        # face_recognition's own library default is 0.6; 0.50 (the old
        # hardcoded default here) is stricter than that, which biases
        # toward more "uncertain"/"unknown" results on real matches --
        # i.e. worse recognition accuracy, not better. Since the right
        # value depends on your actual registered-face dataset (lighting,
        # camera, angle diversity), this is now tunable via env var
        # instead of guessed blind. Try 0.55 or 0.6 and compare the
        # matched/uncertain/unknown rate on a real session before and after.
        self.tolerance = tolerance if tolerance is not None else float(os.environ.get("FACE_MATCH_TOLERANCE", 0.55))
